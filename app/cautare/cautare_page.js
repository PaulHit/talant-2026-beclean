  ...
  return (
    <div className="card border-l-4 border-l-gold-500 fade-up">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="font-display font-bold text-navy-900 text-xl">{participant.nume}</p>
          <p className="text-gray-500 text-sm mt-1">{participant.categorie}</p>
        </div>
        <div className="flex-shrink-0 text-right">
          <p className="text-xs text-gray-400 uppercase tracking-wide">Locul</p>
          <p className="font-display font-bold text-3xl text-gold-500">{participant.loc}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="mb-8 fade-up">
        ...
        <p className="text-gray-500">
          Introdu numele tău pentru a vedea în ce sală și la ce loc ești distribuit.
        </p>
      </div>
        ...
        <div>
          {results.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-navy-700 font-semibold">Niciun participant găsit</p>
              <p className="text-gray-400 text-sm mt-1">
                Verifică dacă ai scris corect numele. Poți căuta și doar după prenume sau
                doar după nume.
              </p>
            </div>
          ) : (
      ...
      {results === null && !error && !loading && (
        <div className="text-center py-16 text-gray-300">
          ...
          <p className="text-gray-400">Introdu un nume pentru a începe căutarea</p>
        </div>
      )}
    </div>
  );
